...
# Ass wird immer als High Card behandelt, d.h. Ass-2-3-4-5 ist keine Strasse
...
logPath = path.join(path.dirname(__file__), "pypoker.log")
...
class Window(Frame):
    ...
    def playButton(self):
        ...
        if self.ErsteRunde:
            ...
            ersteHand = ",".join([f"{self.cards[t[1]].Color}-{self.cards[t[1]].Value}" for t in self.cardList])
            self.logMessage = f"{uhrzeit}: Erste Hand: {ersteHand} Einsatz: {einsatz}"
        self.KartenAnzeigen()
        if not self.ErsteRunde:
            ...
            zweiteHand = ",".join([f"{self.cards[t[1]].Color}-{self.cards[t[1]].Value}" for t in self.cardList])
            self.logMessage += f" Zweite Hand: {zweiteHand} Ergebnis: {hand} Jettons: {jettons}\n"
            with open(logPath, "a") as fh:
                fh.write(self.logMessage)
            for btn in self.btnList:
                btn.configure(bg = "SystemButtonFace")
