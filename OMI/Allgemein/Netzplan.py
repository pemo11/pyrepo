...
# Beispiel 1 (aus Datei einlesen)
# netzplan["A"] = Prozess("A", 2, "", "B")
# netzplan["B"] = Prozess("B", 4, "A", ("C", "D"))
# netzplan["C"] = Prozess("C", 3, "B", ("E", "F"))
# netzplan["D"] = Prozess("D", 2, "B", "E")
# netzplan["E"] = Prozess("E", 1, ("C", "D"), "G")
# netzplan["F"] = Prozess("F", 4, "C", "G")
# netzplan["G"] = Prozess("G", 5, ("E", "F"), "")
...
def ErstelleNetzplan(Dateipfad):
    ...
    with open(Dateipfad, encoding="Utf-8") as fh:
        for zeile in fh:
            ...
            vor = vor.split(":")
            nach = nach.split(":")
            netzplan[name] = Aufgabe(name,beschreibung,dauer,vor,nach)

    # Schritt 1: FAZ und FEZ im Vorwärtsgang berechnen
    for pk in netzplan:
        p = netzplan[pk]
        # Gibt es mehrere Vorgänger?
        # if len(p.Vorgaenger) == 1:
    ...
    # Also alle Aufgaben, bei denen GP und FP 0 sind
    kritischerPfad = []
    # Das erste Element holen
    # Weitere Randbedingung  - gibt es mehrere Aufgaben ohne Vorgänger, hole den mit GP=0
    ...
    while p.AnzahlNachfolger > 0:
        ...
        # if len(p.Nachfolger) == 1 and p.GP == 0 and p.FP == 0:
